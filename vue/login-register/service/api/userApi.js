...
router.post('/login',(req,res)=>{
	const user = req.body;
	const sel_mail = $sql.user.select + " where mail = '" + user.mail + "'";
	console.log(sel_mail);
	conn.query(sel_mail, user.mail, (error, results)=>{
		if (error) {
			throw error;
		}
		console.log(results)
		if (results[0] === undefined) {
			...
			if (results[0].mail == user.mail && results[0].password == user.password) {
...
router.post('/add', (req, res) => {
	...
	conn.query(sel_sql, params.username, (error, results) => {
		...
		if (results.length != 0 && params.username == results[0].username) {
			...
			conn.query(add_sql, [params.username, params.mail, params.password], (err, rst) => {
				if (err) {
					console.log(err);
				} else{
					console.log(rst);
